...
const adminDoc = makeDoc([
  ...
  note("承認すると、出展社の組織とログインアカウントが自動作成され、初期パスワードが一度だけ画面に表示されます。この画面を閉じると再表示できないため、必ずコピーして出展社の担当者へ安全な方法でお伝えください。"),
  spacer(),

  h1("10. ユーザー管理"),
  p("組織（出展社・協力会社など）とログインユーザーを管理します。"),
  img("admin_users", 600),
  bullet("ユーザー作成時は初期パスワードが自動発行され、一度だけ表示されます"),
  ...
  bullet("通知が来ない → ベルの通知は画面表示中に約1分間隔で自動更新されます。メール通知は今後追加予定です"),
