...
const exhibitorDoc = makeDoc([
  ...
  h1("7. メールでの提出"),
  p("Webの操作が難しい場合は、書類をメールに添付して送るだけでも提出できます。宛先は次のアドレスです。"),
  p("提出用アドレス: hub@dosl.co.jp"),
  bullet("このシステムに登録されているご自身のメールアドレスから送ってください（別のアドレスからは受け付けられません）"),
  bullet("件名に書類の種類を入れてください（例:「電気申込 3コマ分」）。複数の展示会が開催中の場合は、展示会名も件名に入れてください（例:「○○展示会 電気申込」）"),
  bullet("添付できるのはWeb提出と同じ形式（Excel・Word・PDF・写真、50MBまで）です"),
  bullet("送信後、数分以内に自動で取り込まれ、「提出済み書類」にメールのマークつきで表示されます"),
  spacer(),
...
const adminDoc = makeDoc([
  ...
  note("承認すると、出展社の組織とログインアカウントが自動作成され、初期パスワードが一度だけ画面に表示されます。初期パスワードは本人宛にメールでも自動送信されます（差出人: DOSL HUB <hub@dosl.co.jp>・画面に「✓ メールでも送信済み」と表示）。メールが届かない場合に備え、画面のパスワードもコピーして控えてください。"),
  spacer(),

  h1("10. ユーザー管理"),
  p("組織（出展社・協力会社など）とログインユーザーを管理します。"),
  img("admin_users", 600),
  bullet("ユーザー作成時は初期パスワードが自動発行され、一度だけ表示されます（本人宛にメールでも自動送信されます）"),
  ...
  new Paragraph({ pageBreakBefore: true, heading: HeadingLevel.HEADING_1, children: [new TextRun("12. メールでの書類受付")] }),
  p("Webが苦手な出展社は、専用アドレス hub@dosl.co.jp へ書類を添付して送るだけで提出できます（稼働中）。次のように動きます。"),
  ...
  bullet("開催中の展示会が複数ある場合、どの展示会宛か件名で判定します。判定できないメールは保留され通知が届くので、出展社に件名へ展示会名を入れて再送するよう依頼してください"),
  bullet("カテゴリを判定できなかった書類は「メール受信（未分類）」に入ります。書類の詳細を開くと下のような振替欄が出るので、正しいカテゴリへ振り替えてください"),
  img("admin_doc_transfer", 560),
  note("同じメールが二重に取り込まれることはありません（処理済みメールは自動で既読になります）。出展社への案内文例: 「書類は hub@dosl.co.jp へ添付してお送りください。件名に展示会名と書類の種類（例: 電気申込）をご記入ください。」"),
  spacer(),
