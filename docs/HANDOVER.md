...
| 3 | ファイル保存のGCS化 | **コード実装済み（2026-07-08・storage.py抽象レイヤ・localバックエンドで実ブラウザ検証済み）**。バケット gs://dosl-hub-documents 作成済み。**本番切替は未**＝残り4手順（①VM SAへのIAM付与 ②VMスコープ変更=停止起動で数分ダウンタイム ③デプロイ+pip install ④.envにSTORAGE_BACKEND=gcs/GCS_BUCKET追記→本番E2E検証）。手順は指示書03 |
...
| 7 | メール受信 (Phase 2) | **コード実装済み（2026-07-08・services/mail_ingest.py・pytest 11件＋テストDBで取込3パターン実ブラウザ検証済み）**。5-4のメール送信（mail_sender.py・MAIL_SEND_ENABLED既定OFF）も同時実装。**残り＝専用Gmailアカウントの用意→backend/scripts/gmail_auth.py でトークン取得→本番.envに5変数追記＋ALTER（source_message_id）＋デプロイ→実メールE2E** |
