# 【全件実装済み】実装指示書アーカイブ（設計参照用）

> **⚠ 2026-08-09 棚卸し: この5本はすべて実装済み・本番稼働中**（01=c7f86c7 / 02=9737e9b / 03=14c841e / 04=3b55b89 / 05=7b9098f、完了記録=e6b99e9）。
> **指示書の記述は実装前の状態のまま**なので、「残タスク」として再実装しないこと。
> 特に02の「最大20ファイル」は、その後のセキュリティ修正（4e00d7c）で**5ファイル・リクエスト60MBに引き下げ済み**。指示書の数値でコードを触ると退行になる。
> このフォルダは設計意図の参照用として残す。現状の正は HANDOVER.md とコード本体。
