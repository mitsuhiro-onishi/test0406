...
class Document(Base):
    ...
    submission_category_id: Mapped[uuid.UUID] = mapped_column(Uuid, ForeignKey("submission_categories.id"))
    booth_id: Mapped[uuid.UUID | None] = mapped_column(Uuid, ForeignKey("booths.id"))
    uploaded_by_org_id: Mapped[uuid.UUID] = mapped_column(Uuid, ForeignKey("organizations.id"))
    uploaded_by_user_id: Mapped[uuid.UUID | None] = mapped_column(Uuid, ForeignKey("users.id"))
    recipient_org_id: Mapped[uuid.UUID] = mapped_column(Uuid, ForeignKey("organizations.id"))
    ...
    ai_analyses = relationship("AIAnalysis", back_populates="document", order_by="desc(AIAnalysis.created_at)")
